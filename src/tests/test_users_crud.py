...
def test_get_auth0():
    with patch("app.crud.user.GetToken") as token_cls, \
         patch("app.crud.user.Auth0") as auth0_cls:
...
def test_get_roles_per_user(auth0_mock, sample_auth0_user):
    ...
    with patch("app.crud.user.get_auth0", return_value=auth0_mock):
...
async def test_update_user(auth0_mock, sample_auth0_user):
    ...
    with patch("app.crud.user.get_auth0", return_value=auth0_mock), \
         patch("app.crud.user.get_user_by_id", AsyncMock(return_value=user)):
