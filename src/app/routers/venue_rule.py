...
@router.get("/venue_rules", response_model=list[VenueRule])
...
@router.patch("/venue_rule", response_model=VenueRule, status_code=201)
async def change_venue_rule(item: VenueRule, db: Session=Depends(get_session),
               _: str = Depends(verify_write_venue_rules)):
...
@router.get("/venue_rule/{id}", response_model=VenueRule)
